...
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False, mods=None):
    ...
    g187 = np.random.normal(7.0, 1.0, n_points)  # датчик 1 дросселя (ХХ ~7%)
    g188 = 100.0 - g187 + np.random.normal(0, 0.3, n_points)  # датчик 2 зеркальный
    g79 = np.ones(n_points) * 14.0  # педаль отпущена
    g185 = g79 / 2.0
    throttle = g187  # для графика угла дросселя
...
- 014-4 (Статус распознавания): ON (система активна). OFF — ЭБУ временно отключил подсчёт.
...
1. Убедись, что ошибки в ЭБУ удалены, двигатель заглушен, зажигание включено, температура ОЖ от 5°C до 90°C (лучше на холодную), педаль газа полностью отпущена.
2. В VCDS: 01-Двигатель → Базовые параметры (04) → Группа 060 → Нажать «Прочитать» (Go!).
3. Дождаться, пока статус сменится с ADP. RUN на ADP. OK (заслонка будет жужжать).
4. Выйти из группы, выключить зажигание на 20 секунд, затем завести двигатель.
"""
