...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (с поддержкой модов) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False, mods=None):
    ...
    # Параметры группы 041/046
    o2_heater_resistance = np.random.normal(8.0, 2.0, n_points)  # 2-15 Ом
    cat_conversion = np.random.normal(0.2, 0.1, n_points)        # 0.0-0.45 норма
    cat_status = np.ones(n_points)  # 1 = OK

    if mods.get("decatted", False):
        # При удалённом катализаторе тест отключён
        cat_status = np.full(n_points, -1)  # TEST OFF
        cat_conversion = np.zeros(n_points)  # не измеряется
        # Сопротивление подогрева Зонда 2 не показываем, Зонд 1 в норме
    ...
    if scenario == "detonation":
        ...
        dd_c3 = dd_base + np.random.normal(0, 0.05, n_points)
        dd_c4 = dd_base + np.random.normal(0, 0.05, n_points)
        cat_conversion = np.random.normal(0.5, 0.2, n_points)  # под нагрузкой может ухудшиться
    elif scenario == "leak":
        ...
        cat_conversion = np.random.normal(0.7, 0.2, n_points)  # богатая смесь вредит кату
        ...
        dd_c3 = dd_base + np.random.normal(0, 0.05, n_points)
        dd_c4 = dd_base + np.random.normal(0, 0.05, n_points)
        cat_conversion = np.linspace(0.4, 0.8, n_points)  # ухудшение
        ...
        o2_heater_resistance = np.full(n_points, 99.9)  # обрыв подогрева
        ...
        cat_conversion[:50] = 0.7  # плохая конверсия на ХХ
    ...
    df = pd.DataFrame({
        ...
        "Статус катализатора": cat_status.astype(int),  # 1=OK, 0=NOT OK, -1=TEST OFF
    })
    return df

# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (ФИНАЛ) ---
...
- УДАЛЕН КАТАЛИЗАТОР (Евро-2): Вторая лямбда (Группа 041-1) может быть прямой линией, повторять первую или висеть в ошибке. Игнорируй P0420, не предлагай замену ката.
  Сопротивление и статус подогрева Зонда 2 игнорируются. Если при включённом флаге в логе всё равно есть статус Cat B1 - NOT OK или ошибка P0420 — прошивка Евро-2 выполнена некорректно.
"""
    else:
        mods_note += """
...
"""
    if mods.get("lpg", False):
        mods_note += """
- УСТАНОВЛЕНО ГБО (Газ): LTFT (032) в пределах ±8% — НОРМА. Не ставь диагноз 'подсос' или 'умирающий бензонасос' при таких коррекциях. Следи за пропусками из-за газовых форсунок.
  На газе откаты УОЗ (020) должны быть строго 0.0° даже при интенсивном разгоне. Если появляются откаты — газовая смесь слишком бедная (проверить редуктор, газовые фильтры).
"""
    if mods.get("tuned", False):
        mods_note += """
- ЧИП-ТЮНИНГ (Агрессивная прошивка): УОЗ (003-4) может быть более ранним, время впрыска (002-3) выше стоковых 3.0 мс под нагрузкой. Оценивай стабильность, а не абсолютные значения.
  При агрессивной прошивке откаты в Группе 020 могут постоянно висеть в максимальных лимитах (до -9°..-12°). Это признак некачественной настройки или использования низкооктанового топлива. Длительная езда с такими откатами разрушает поршни.
"""
...
- 001-2 (Температура ОЖ): Рабочая = 85–98 °C. Если в логе температура застыла на 70-78°C — термостат заклинил в открытом положении.
- 001-3 (Лямбда-регулирование): Норма = ±10.0% (должно циклически колебаться). Застывание в 0.0% или крайних лимитах указывает на отказ первого лямбда-зонда.
- 001-4 (Бинарный статус): 11111111 — все тесты пройдены.

ГРУППА 002 (Воздух, Впрыск и Нагрузка):
- 002-2 (Нагрузка двигателя): Для базовой комплектации (МКПП, без AC) норма = 15.0–18.5%. Значения > 20.0% на ХХ — аномалия.
- 002-3 (Время впрыска): Норма = 2.00–3.00 мс. Если > 3.20 мс при нормальном MAP — форсунки загрязнены или низкое давление топлива.
- 002-4 (Давление MAP / ДАД): Для базовой комплектации норма = 280–300 мбар. Если значение > 315 мбар на ХХ — это жесткий триггер неисправности (сверяй с углами дросселя 003-3 и коррекциями 032-1 для поиска подсоса или смещения ГРМ).
...
- 033-1 (Мгновенная лямбда-коррекция, STFT онлайн): ±10.0%. Зависание в +25% или -25% — выход за лимит регулирования.

ГРУППА 003 (Зажигание и дроссель):
- 003-1 (Обороты): 650–750 об/мин (дублирует 001-1).
- 003-2 (MAP): 280–300 мбар. Дублирует 002-4. В базовой комплектации жесткий лимит < 315 мбар.
- 003-3 (Угол дроссельной заслонки): 1.0–3.5% на прогретом ХХ без потребителей.
  - > 3.5% — заслонка обросла нагаром, требуется чистка + адаптация (группа 060).
  - < 1.0% при высоком MAP — явный подсос воздуха в обход заслонки.
- 003-4 (Угол опережения зажигания): 4.0° … 12.0° до ВМТ на ХХ. Постоянно скачет (например, 4.5° → 7.0° → 11.5°) — это норма, ЭБУ поддерживает стабильный холостой ход.

ГРУППЫ 014, 015, 016 (Пропуски):
- Суммарные пропуски (014-3): 0.
- Счетчики по цилиндрам (015/016): строго 0.

ГРУППЫ 062, 063 (Дроссель/Педаль):
- 062-1 + 062-2 = 100% (зеркальность датчиков G187 и G188).
- 062-3 = 062-4 * 2 (датчики педали G79 и G185, соотношение 2:1).

ГРУППА 020 (Контроль детонации):
- 020-1 (Откат УОЗ Цил.1): 0.0° (допуск до -4.5°). На ХХ всегда 0.0.
- 020-2 (Откат УОЗ Цил.2): аналогично.
- 020-3 (Откат УОЗ Цил.3): аналогично.
- 020-4 (Откат УОЗ Цил.4): аналогично.
Примечание: Группы 021 и 022 часто неактивны или дублируют 020. Основной анализ ведётся по Группе 020.

ГРУППА 021 (Резерв / Напряжение ДД): В большинстве версий ПО дублирует 020 или 026.
ГРУППА 022 (Резерв / Статус регулирования): Информационное поле.

ГРУППА 026 (Напряжение датчиков детонации):
- 026-1...026-4 (Вольтаж Цил 1-4): На ХХ = 0.40–0.90 В, под нагрузкой = 1.50–3.50 В. Растёт плавно с оборотами.
- Разброс между цилиндрами на ХХ не должен превышать 0.20 В.

ГРУППА 093 (Синхронизация фаз ГРМ):
- 093-3 (Фазовое положение Bank 1): Идеал = 0.00°. Допуск = от -3.00° до +3.00°.
- Если значение ниже -4.00° — цепь ГРМ растянута, требуется подготовка к замене.
- Если значение ниже -5.50° или выше +5.50° в сочетании с MAP > 360 мбар — критическое растяжение или проскок цепи на 1 зуб. Эксплуатация опасна!
- Если значения в 093-3 резко меняются после перегазовок (например, с минуса на плюс) — трактуй как люфт/износ шпонки шестерни коленвала.

ГРУППА 099 (Статус лямбда-регулирования):
- 099-3 (Текущая коррекция): ±10.0% (должна постоянно меняться).
- 099-4 (Статус контроля): В норме на прогретом моторе (>80°C) должен быть строго ON (Вкл).
- Если статус равен OFF (Выкл) на горячем ХХ — ЭБУ работает в аварийном режиме (Open Loop) из-за неисправности первого зонда или некорректного чип-тюнинга Евро-2.

ГРУППЫ 041, 046 (Подогрев и Катализатор):
- 041-1 (Сопротивление подогрева Зонда 1): Норма = 2.0–15.0 Ом. Значения > 50 Ом — обрыв спирали подогрева датчика.
- 046-3 (Конверсия катализатора): Норма = 0.00–0.45. Если > 0.60 на стоке — катализатор разрушен (эффективность ниже предела).
- 046-4 (Статус ката): Норма = Cat B1 - OK (1). Если при включенном флаге decatted=True в логе все еще активен статус NOT OK (0) или висит ошибка P0420 — прошивка Евро-2 сделана некорректно.

--- ЛОГИКА КРОСС-ВАЛИДАЦИИ (расширенная) ---
1. ПРОБОЙ КАТУШКИ/СВЕЧИ: ЕСЛИ пропуски быстро растут ТОЛЬКО в одном цилиндре И мгновенная лямбда (033-1) уходит в плюс, ТОГДА: Локальный пропуск. Переставить катушку на другой цилиндр.
2. ПРОПУСКИ ИЗ-ЗА БЕДНОЙ СМЕСИ: ЕСЛИ пропуски хаотичны по всем цилиндрам И коррекции (032-1/2) > +8.0%, ТОГДА: Системное обеднение (бензонасос или подсос).
3. ПОТЕРЯ КОМПРЕССИИ: ЕСЛИ постоянные пропуски в ОДНОМ цилиндре ТОЛЬКО на ХХ (при >2000 об/мин счетчик стоит) И MAP > 360 мбар, ТОГДА: Механическая потеря компрессии. Замер компрессометром.
...
with st.sidebar:
    ...
    if st.button("⚡ Сгенерировать лог"):
        st.session_state.generated_log_df = generate_test_log_df(
            ...
            mods=st.session_state.mods  # передаём моды
        )
        st.rerun()
    ...
    with st.expander("📊 Посмотреть график параметров", expanded=True):
        if st.session_state.diagnostic_mode.startswith("Электрика"):
            ...
            selected_cols = [c for c in log_df.columns if "Давление" in c or "коррекция" in c
                             ...
                             or "Температура ОЖ" in c or "Фазовое положение" in c
