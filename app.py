...
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)"):
    ...
    if scenario == "detonation":
        ...
        stft = np.linspace(2.0, 24.0, n_points) + np.random.normal(0, 1.0, n_points) # Лямбда уходит в жесткую бедную смесь
        ltft = np.zeros(n_points) + 8.5 # Мультипликатив выше нормы (>7%)
        ...
        g188 = np.ones(n_points) * 70.0 
        rpm = 1500 + np.random.normal(0, 10, n_points) 
    elif scenario == "throttle_jam":
        g79 = np.linspace(14.5, 90.0, n_points)
        g185 = g79 / 2
        g187 = np.clip(np.linspace(4.5, 88.0, n_points), 0, 35.0) 
        g188 = 100 - g187
        rpm = np.linspace(840, 2500, n_points) 
...
"""
    ...
    mechanics_rules = """--- БАЗА ЭТАЛОНОВ ДВИГАТЕЛЯ CFNA 1.6 MPI (МЕХАНИКА, ТОПЛИВО И ДРОССЕЛЬ) ---
ГРУППА 001:
...
- 002-4 (ДАД / MAP): 280–340 мбар. Если >360 мбар — подсос воздуха, проскок цепи ГРМ.

ГРУППА 003 (Дроссель и Зажигание):
- 003-3 (Угол заслонки): 1.0–3.5%. Если >4.0% — грязная, требует чистки.
- 003-4 (УОЗ): от 4.0° до 12.0° до ВМТ.

ГРУППЫ 032, 033 и 106 (Смесеобразование и Топливо):
- 032-1 (Аддитив, ХХ): -3.0% ... +3.0% (Лимит ±10%). > +4.0% = подсос (ВКГ, кольца форсунок). < -4.0% = избыток топлива (текут форсунки).
- 032-2 (Мультипликатив, Нагрузка): -5.0% ... +5.0% (Лимит ±10%). > +6.0% = нехватка топлива на оборотах (насос, фильтр). < -6.0% = перелив (заклинил РДТ).
- 033-1 (Мгновенная лямбда): -10.0% ... +10.0%. Зависло в +25% = критически бедная смесь. Зависло в -25% = богатая.
- 033-2 (Вольтаж Лямбда 1): 1.40 ... 2.10 В (Стехиометрия 1.50 В).
- Группа 106 (Давление топлива): На CFNA датчика давления нет! Значение может быть N/A или показывать ШИМ (40-65%). Основной упор делать на группы 032 и 033.
...
- 063-2 (Кик-даун АКПП): 0% (Выкл) -> 100% (Вкл).

--- ЛОГИКА КРОСС-ВАЛИДАЦИИ ДЛЯ ИИ ---
1. СКРЫТЫЙ ПОДСОС ВОЗДУХА: ЕСЛИ 032-1 (Аддитив) > +5.0% И 032-2 в норме (±2.0%) И MAP > 350 мбар, ТОГДА: Подсос за дроссельной заслонкой (клапан ВКГ, кольца форсунок).
2. УМИРАЮЩИЙ БЕНЗОНАСОС / ФИЛЬТР: ЕСЛИ 032-1 в норме И 032-2 > +7.0% И 033-1 при разгоне уходит в +20..+25% (Время впрыска > 4.0 мс), ТОГДА: Дефицит топлива под нагрузкой. Рекомендовать замер давления манометром (норма 4.0 бар).
3. ПЕРЕЛИВ ТОПЛИВА (Форсунки/РДТ): ЕСЛИ 032-1 < -5.0% ИЛИ 032-2 < -6.0% И Вольтаж 033-2 стабильно < 1.1 В, ТОГДА: Текущие форсунки, заклинил регулятор давления (давление >4.5 бар).
4. НАРУШЕНИЕ КОРРЕЛЯЦИИ ЗАСЛОНКИ (P0121/P0221): ЕСЛИ 062-1 + 062-2 != 100%, ТОГДА: Износ дорожек или окисление контактов заслонки. Требуется чистка и адаптация (060).
...
with st.sidebar:
    ...
    if st.session_state.diagnostic_mode == "Электрика и CAN (Группы 125-135)":
        ...
        test_scenario = st.selectbox(
            "Сценарий (Механика):",
            ["Исправный мотор", "Подсос воздуха", "Пропуски (Цилиндр 2)", "Детонация (Откаты УОЗ)", "Богатая смесь (-20%)", "Умирающий бензонасос", "Рассинхрон дросселя (Сумма != 100%)", "Клин заслонки (Педаль в пол)"]
        )
        mapping = {
