...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (ОБНОВЛЁН) ---
...
with st.sidebar:
    ...
    if new_mode != st.session_state.diagnostic_mode or new_trim != st.session_state.is_base_trim:
        ...
        # Обновляем системный промпт с текущими модами
        if st.session_state.chat_history and st.session_state.chat_history[0]["role"] == "system":
            st.session_state.chat_history[0]["content"][0]["text"] = get_system_prompt(
                new_mode, new_trim, mods=st.session_state.mods
            )
        st.rerun()
    ...
    is_tuned = st.checkbox("⚙️ Чип-тюнинг", value=st.session_state.mods["tuned"])
    is_decatted = st.checkbox("💨 Удален катализатор", value=st.session_state.mods["decatted"])
    is_lpg = st.checkbox("🔥 Установлено ГБО", value=st.session_state.mods["lpg"])

    # Обновляем состояние модов и сразу меняем промпт, если что-то изменилось
    new_mods = {"tuned": is_tuned, "decatted": is_decatted, "lpg": is_lpg}
    if new_mods != st.session_state.mods:
        st.session_state.mods = new_mods
        # Пересобираем системный промпт с новыми модами
        if st.session_state.chat_history and st.session_state.chat_history[0]["role"] == "system":
            st.session_state.chat_history[0]["content"][0]["text"] = get_system_prompt(
                st.session_state.diagnostic_mode, st.session_state.is_base_trim,
                mods=new_mods
...
if log_df is not None and not log_df.empty:
    ...
    # Кнопка экспертного анализа (ОБНОВЛЕНО)
