...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (без изменений, уже включает коррекции) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False, mods=None):
    ...
    o2_voltage = np.random.normal(1.50, 0.05, n_points)  # Широкополосный зонд
...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (обновлены группы 032/033) ---
...
"""
    if mods.get("lpg", False):
        mods_note += """
- УСТАНОВЛЕНО ГБО (Газ): Для газа долговременные коррекции (032) в пределах ±8.0% являются допустимой нормой. Не ставь диагноз 'подсос' или 'умирающий бензонасос' только на основании этих значений. Рекомендуй проверить настройку газовой карты.
"""
...
... (остальные паттерны 7-26 остаются без изменений, как в предыдущей версии) ...
"""
...
# --- ИНИЦИАЛИЗАЦИЯ СОСТОЯНИЙ (как раньше) ---
