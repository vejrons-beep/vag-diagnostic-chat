...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (добавлен угол дросселя) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False):
    ...
    uoz = np.random.normal(6.0, 2.0, n_points)  # скачущий УОЗ на ХХ ~4-12°
    throttle = np.random.normal(2.2, 0.2, n_points)  # угол дросселя ~2-3% на ХХ
    ...
    if scenario == "detonation":
        ...
        throttle = np.clip(g79 * 0.7 + np.random.normal(0, 2, n_points), 0, 100)  # пропорционально педали
    elif scenario == "leak":
        ...
        throttle = np.random.normal(0.8, 0.2, n_points)  # заслонка почти закрыта из-за подсоса
    elif scenario == "rich":
...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (добавлены детали по 003 группе) ---
...
"""
    if mods.get("tuned", False):
        mods_note += """
- ЧИП-ТЮНИНГ: УОЗ (003-4) может быть более ранним, время впрыска (002-3) выше стоковых 3.0 мс под нагрузкой. Оценивай стабильность, а не абсолютные значения.
"""
...
- 003-1 (Обороты): 650–750 об/мин (дублирует 001-1 для синхронизации).
...
--- ЛОГИКА КРОСС-ВАЛИДАЦИИ (дополнена паттернами 003 группы) ---
...
if "reference_map" not in st.session_state:
    st.session_state.reference_map = {
        ...
        "Угол дросселя (%)": (1.0, 3.5, "green", "red")   # эталон для ХХ
    }
    ...
    with st.expander("📊 Посмотреть график параметров", expanded=True):
        if st.session_state.diagnostic_mode.startswith("Электрика"):
            ...
            selected_cols = [c for c in log_df.columns if "Давление" in c or "коррекция" in c
                             ...
                             or "Угол опережения" in c or "Угол дросселя" in c]
            if selected_cols:
                fig = go.Figure()
