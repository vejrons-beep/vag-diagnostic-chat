...
if log_df is not None and not log_df.empty:
    ...
    # --- КНОПКА ЭКСПЕРТНОГО АНАЛИЗА (ФИНАЛЬНЫЙ ШТРИХ) ---
    ...
    if st.button(btn_label):
        if not API_KEY:
            ...
            system_instruction = get_system_prompt(st.session_state.diagnostic_mode, st.session_state.is_base_trim)
            ...
            # Не добавляем ответ в историю чата, чтобы не захламлять контекст
