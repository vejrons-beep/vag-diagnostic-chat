...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (без изменений) ---
def get_system_prompt(mode="Механика (Группы 001-063)", is_base_trim=False, ecu_type="Magneti Marelli 7GV", mods=None):
    # ... (весь код get_system_prompt без сокращений, как в предыдущем полном ответе)
    pass
...
if image_base64 is not None:
    ...
    if st.button("👁️ Отправить скриншот"):
        if not API_KEY:
            ...
            save_chat_history(st.session_state.chat_history)   # <-- сохраняем в B1
            st.session_state.uploaded_image_key += 1
            st.rerun()

# --- ЧАТ-ВВОД (история сохраняется в B1, профиль обновляется при обнаружении VIN) ---
if user_input := st.chat_input("Напишите симптомы или задайте вопрос..."):
    if not API_KEY:
        ...
        # Автоматически извлекаем VIN из сообщения и обновляем профиль в A1
