...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (добавлены откаты УОЗ и напряжения ДД) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False):
    ...
    # --- Откаты УОЗ (Группа 020) ---
    knock_c1 = np.zeros(n_points)
    knock_c2 = np.zeros(n_points)
    knock_c3 = np.zeros(n_points)
    knock_c4 = np.zeros(n_points)

    # --- Напряжения датчика детонации (Группа 026) ---
    ...
    if scenario == "detonation":
        ...
        # Детонация – откаты во всех цилиндрах
    ...
    df = pd.DataFrame({
        ...
        "Откат УОЗ Цилиндр 1 (°KW)": np.round(knock_c1, 1),
        "Откат УОЗ Цилиндр 2 (°KW)": np.round(knock_c2, 1),
        "Откат УОЗ Цилиндр 3 (°KW)": np.round(knock_c3, 1),
        "Откат УОЗ Цилиндр 4 (°KW)": np.round(knock_c4, 1),
        "Напряжение ДД Цилиндр 1 (В)": np.round(dd_c1, 2),
        "Напряжение ДД Цилиндр 2 (В)": np.round(dd_c2, 2),
        "Напряжение ДД Цилиндр 3 (В)": np.round(dd_c3, 2),
        "Напряжение ДД Цилиндр 4 (В)": np.round(dd_c4, 2),
    })
    return df

# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (полный набор групп) ---
...
- УСТАНОВЛЕНО ГБО (Газ): LTFT (032) в пределах ±8% — НОРМА. Не ставь диагноз 'подсос' или 'умирающий бензонасос' при таких коррекциях. Следи за пропусками из-за газовых форсунок.
  На газе откаты УОЗ (Группа 020) должны быть строго 0.0° даже при интенсивном разгоне. Если появляются откаты — газовая смесь слишком бедная (проверить редуктор, газовые фильтры).
"""
...
ГРУППА 021 (Резерв / Напряжение ДД): В большинстве версий ПО дублирует 020 или 026. Анализировать по тем же правилам.
ГРУППА 022 (Резерв / Статус регулирования): Информационное поле. Если активно, показывает готовность системы к контролю детонации.
