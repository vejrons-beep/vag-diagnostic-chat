...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (расширенный) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False):
    ...
    coolant_temp = np.clip(20.0 + time * 0.7, 20.0, 90.0)  # прогрев
    ...
    phase_position = np.random.normal(0.0, 0.3, n_points)  # фазы ГРМ
    ...
    if scenario == "detonation":
        ...
        phase_position = -4.5 + np.random.normal(0, 0.3, n_points)  # смещение фаз при подсосе? Обычно нет, но для реализма чуть сдвинем
    elif scenario == "rich":
        ...
        phase_position = -6.0 + np.random.normal(0, 0.5, n_points)  # сильное смещение фаз

    df = pd.DataFrame({
        "Время (сек)": time,
        "Обороты (об/мин)": np.round(rpm, 0),
        "Температура ОЖ (°C)": np.round(coolant_temp, 1),
        "Нагрузка (%)": np.round(rpm * 0.018, 1),  # упрощённо
        ...
        "Лямбда-регулирование (%)": np.round(stft, 2),  # для 001-3
        "G187 (%)": np.round(g187, 1),
        "G79 (%)": np.round(g79, 1),
    })
    return df

# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (ФИНАЛЬНАЯ ВЕРСИЯ) ---
