...
# --- СТАТИСТИКА ДЛЯ ИИ (исправлена фильтрация) ---
def generate_log_summary(df, vin="", is_base_trim=False):
    """
    Формирует компактный статистический отчёт.
    Если is_base_trim=True, исключаем заведомо отсутствующие блоки: АКПП, АБС, Климат.
    """
    summary = []
    ...
    for col in df.columns[1:]:
        # Глобальное исключение отсутствующих в базовой комплектации блоков
...
# --- ЗАПРОС К OpenRouter (без изменений) ---
    ...
    for m in messages:
        ...
        for item in m["content"]:
            if item["type"] == "text":
                ...
                new_content.append({"type": "text", "text": "[Скриншот отправлен ранее]"})
...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (исправлены маркеры отсутствия) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False):
    ...
    if diagnostic_mode.startswith("Электрика"):
        # Для отсутствующих блоков используем -1
        missing_val = -1 if is_base_trim else 1  # если блок есть, он =1
    ...
    # Сборка DataFrame (без G185, G188 для упрощения)
    df = pd.DataFrame({
        "Отметка Времени (сек)": time,
...
# --- СИСТЕМНЫЙ ПРОМПТ (исправлены термины коррекций и добавлено правило для базовой комплектации) ---
def get_system_prompt(mode="Механика (Группы 001-063)", is_base_trim=False):
...
with st.sidebar:
    ...
    if new_mode != st.session_state.diagnostic_mode or new_trim != st.session_state.is_base_trim:
        ...
        if st.session_state.chat_history and st.session_state.chat_history[0]["role"] == "system":
            st.session_state.chat_history[0]["content"][0]["text"] = get_system_prompt(new_mode, new_trim)
        st.rerun()

    st.markdown("---")
    st.subheader("🚗 Идентификация")
    vin_input = st.text_input("VIN-код:", value=st.session_state.vin_code, max_chars=17)
    ...
    if new_mode.startswith("Электрика"):
...
uploaded_file = st.file_uploader("Лог VCDS (.csv/.txt) или скриншот (.png/.jpg)", type=["csv","txt","png","jpg","jpeg"])
...
if log_df is not None and not log_df.empty:
    st.success("📊 Данные лога распознаны.")
    ...
    with st.expander("📊 График параметров", expanded=True):
        if st.session_state.diagnostic_mode.startswith("Электрика"):
            ...
            if selected_cols:
                ...
                fig.update_layout(template="plotly_dark", title="Параметры с допусками")
                st.plotly_chart(fig, use_container_width=True)
        st.dataframe(log_df.head(10))

    # Кнопка анализа
    btn_label = "🚀 Отправить лог на анализ" if uploaded_file is not None else "🧪 Отправить сгенерированный лог"
    if st.button(btn_label):
        if not API_KEY:
            st.error("API-ключ не найден!")
        else:
            # Генерируем сводку с учётом базовой комплектации
            summary_str = generate_log_summary(log_df, st.session_state.vin_code, st.session_state.is_base_trim)
            ...
            st.session_state.chat_history.append(
                {"role": "user", "content": [{"type": "text", "text": f"📎 Загружен лог.\n{summary_str}"}]}
            )
...
if image_base64 is not None:
    ...
    if st.button("👁️ Отправить скриншот"):
        if not API_KEY:
            ...
            prompt = "Распознай ошибки или группы VCDS на скриншоте и дай диагноз."
            if st.session_state.vin_code:
                prompt = f"VIN: {st.session_state.vin_code}. {prompt}"
            msg = {"role": "user", "content": [{"type": "text", "text": prompt},
                                               {"type": "image_url", "image_url": {"url": image_base64}}]}
            st.session_state.chat_history.append(msg)
            temp_hist = st.session_state.chat_history.copy()
            with st.chat_message("assistant"):
                with st.spinner("Смотрю скриншот..."):
...
# --- ЧАТ-ВВОД ---
if user_input := st.chat_input("Опишите симптомы или задайте вопрос..."):
    if not API_KEY:
        st.error("API-ключ не найден!")
    else:
        ...
        mods_str = ", ".join([k for k,v in mods.items() if v]) or "Сток"
        vin_str = st.session_state.vin_code or "Не указан"
        base_str = "Да" if st.session_state.is_base_trim else "Нет"

        ai_text = (f"[Контекст: {st.session_state.diagnostic_mode}. VIN: {vin_str}. "
                   f"Моды: {mods_str}. Базовая компл.: {base_str}] {user_input}")

        st.session_state.chat_history.append({"role": "user", "content": [{"type": "text", "text": user_input}]})
        temp_history = st.session_state.chat_history.copy()
        temp_history[-1] = {"role": "user", "content": [{"type": "text", "text": ai_text}]}
