...
import base64
import os
import copy  # ИСПРАВЛЕНО: глубокое копирование
...
# ИСПРАВЛЕНО: счётчик для сброса file_uploader
...
with st.sidebar:
    ...
    if st.button("🗑️ Очистить историю"):
        ...
        st.session_state.generated_log_df = None
        st.session_state.uploaded_image_key += 1   # ИСПРАВЛЕНО
...
# ИСПРАВЛЕНО: ключ счётчика
uploaded_file = st.file_uploader(
    "Загрузи лог VCDS (.csv, .txt) ИЛИ Скриншот (.png, .jpg)",
        ...
        if st.session_state.diagnostic_mode.startswith("Электрика"):
            ...
            if selected_cols:
                ...
                for col in selected_cols:
                    ...
                    # ИСПРАВЛЕНО: защита от нестандартных колонок
                    try:
...
if image_base64 is not None:
    ...
    if st.button("👁️ Отправить скриншот"):
        if not API_KEY:
            ...
            # ИСПРАВЛЕНО: глубокое копирование
            ...
            save_history_to_disk(st.session_state.chat_history, st.session_state.vin_code)
            # ИСПРАВЛЕНО: сброс загрузчика, чтобы кнопка исчезла
...
if user_input := st.chat_input("Напишите симптомы или задайте вопрос..."):
    if not API_KEY:
        ...
        # Сохраняем чистое сообщение пользователя
        ...
        with st.chat_message("assistant"):
            with st.spinner("Думаю..."):
                response = ask_ai_chat(API_KEY, MODEL_NAME, temp_history, max_tokens=1500)  # ИСПРАВЛЕНО: увеличен лимит
                st.write(response)
