...
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=True):
    ...
    if scenario == "detonation":
        ...
        stft = np.linspace(0, 12.0, n_points) + np.random.normal(0, 1.0, n_points) # Лямбда реагирует на кислород
        ...
        misfire_c4 = np.concatenate([np.cumsum(np.random.choice([0, 1], p=[0.5, 0.5], size=50)), np.ones(50)*25]) # Растут на ХХ, замирают на оборотах
...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ В ЗАВИСИМОСТИ ОТ РЕЖИМА И КОМПЛЕКТАЦИИ ---
...
- 032-1 (Аддитив): ±3.0%. > +4.0% = подсос. < -4.0% = перелив форсунок.
- 032-2 (Мультипликатив): ±5.0%. > +6.0% = нехватка топлива (насос/фильтр).
...
if "chat_history" not in st.session_state:
    if saved_history:
        ...
        st.session_state.chat_history = [
            {"role": "system", "content": [{"type": "text", "text": get_system_prompt(st.session_state.diagnostic_mode, st.session_state.is_base_trim)}]},
...
with st.sidebar:
    ...
    if new_mode != st.session_state.diagnostic_mode or new_trim != st.session_state.is_base_trim:
        ...
        if len(st.session_state.chat_history) > 0 and st.session_state.chat_history[0]["role"] == "system":
            ...
            st.session_state.chat_history.append({"role": "assistant", "content": [{"type": "text", "text": f"🔄 Конфигурация обновлена: Режим **{new_mode}**, Базовая комплектация: **{'Да' if new_trim else 'Нет'}**."}]})
        st.rerun()
...
if log_df is not None and not log_df.empty:
    ...
    with st.expander("📊 Посмотреть график параметров", expanded=True):
        if st.session_state.diagnostic_mode == "Электрика и CAN (Группы 125-135)":
            ...
            selected_cols = [c for c in log_df.columns if "Давление" in c or "коррекция" in c or "Пропуски" in c or "Откат" in c]
            if selected_cols:
                fig = go.Figure()
...
if user_input := st.chat_input("Напишите симптомы или задайте вопрос..."):
    if not API_KEY:
        ...
        ai_text_payload = f"[Контекст: {st.session_state.diagnostic_mode}. VIN: {vin_str}. Моды: {mods_str}. Базовая компл.: {base_trim_str}] {user_input}"
