...
# --- ГЕНЕРАТОР ТЕСТОВЫХ ЛОГОВ (добавлена температура впуска) ---
def generate_test_log_df(scenario="normal", diagnostic_mode="Механика (Группы 001-063)", is_base_trim=False, mods=None):
    ...
    # Температура впуска: чуть выше уличной, на ХХ поднимается
...
# --- ДИНАМИЧЕСКИЙ СИСТЕМНЫЙ ПРОМПТ (ФИНАЛ + 134) ---
...
1–6 (базовые паттерны пропусков, подсоса, бензонасоса, дросселя).
7–9 (заслонка, подсос по дросселю, износ шестерен).
10–12 (детонация: плохое топливо, бедная смесь, локальный стук).
13–15 (механический стук, обрыв ДД, внешний шум).
16–17 (загрязненные форсунки, дифференциация подсос/ГРМ).
18–19 (термостат, неисправный лямбда-зонд).
20–21 (критический износ цепи ГРМ, люфт шпонки коленвала).
22 (аварийный режим лямбда-регулирования).
23–25 (обрыв подогрева зонда, неэффективный катализатор, кривая прошивка Евро-2).
