...
По умолчанию основным автомобилем для диагностики является Volkswagen Polo Sedan 2012 г.в., двигатель 1.6 CFNA (16V, цепь ГРМ, распределенный впрыск). Если VIN не указан или относится к этой линейке, always применяй жесткие заводские эталоны именно для CFNA.
...
- Если VIN-код не предоставлен, не распознан или его недостаточно для определения точных параметров двигателя, ты НЕ должен угадывать технические характеристики.
- Вместо этого ты обязан вежливо задать пользователю конкретные наводящие вопросы по цепочке для сбора паспорта автомобиля:
    ...
    2. Количество клапанов на цилиндр или всего (8V / 16V).
    3. Привод ГРМ (цепь / ремень).
    4. Тип впрыска (распределенный MPI / прямой TSI, FSI).
    5. Наличие наддува (атмосферный мотор / есть турбина или компрессор).
    6. Тип коробки передач (механика / классический автомат / робот DSG).
    7. Тип системы зажигания (индивидуальные катушки на каждый цилиндр / общая катушка-модуль с высоковольтными проводами).
- Ты можешь задавать эти вопросы не все сразу, а сгруппировав их для удобства пользователя.
- Как только пользователь предоставляет эти данные, ты фиксируешь их в контексте диалога и мгновенно адаптируешь свои диагностические алгоритмы, эталоны давления ДАД/турбины, нормы времени впрыска и ожидаемые группы VCDS под указанную конфигурацию.

6. ОБЩИЕ ПРИНЦИПЫ КОММУНИКАЦИИ:
- БЕЗОПАСНОСТЬ: При признаках критических проблем (сильная детонация, перегрев) предупреждай сразу.
- ДИАЛОГ: Веди диагностику как живой мастер-приёмщик — задавай уточняющие вопросы по симптомам и поведению машины, если в логах или описании мало контекста."""
...
if user_input := st.chat_input("Напишите симптомы или задайте вопрос..."):
    if not API_KEY:
        ...
        # Формируем скрытый payload для ИИ: если VIN есть, добавляем его к тексту
        ai_text_payload = user_input
        if st.session_state.vin_code:
            ai_text_payload = f"[Контекст системы - VIN автомобиля: {st.session_state.vin_code}] {user_input}"
            
        # В историю чата для отображения на экране пишем чистый текст пользователя, чтобы не засорять интерфейс
        new_message = {"role": "user", "content": [{"type": "text", "text": user_input}]}
        st.session_state.chat_history.append(new_message)
        
        # А вот для отправки в API OpenRouter подменяем последнее сообщение на payload с VIN-кодом
        temp_history = st.session_state.chat_history.copy()
        temp_history[-1] = {"role": "user", "content": [{"type": "text", "text": ai_text_payload}]}
        
        with st.chat_message("assistant"):
            with st.spinner("Думаю..."):
                # Передаем историю со скрытым VIN-кодом
